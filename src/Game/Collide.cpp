...
void Game::collideMonster(void)
{
    if(collideTwoSprite64x64(m_Player.getPosition(), m_Mob1.getPosition()))
    {
        if(m_Mob1.isAlive())
        {
            ...
            {
                if(collide == DOWN_COLLIDE && !previewCollide(m_Player.getPosition(), sf::Vector2f(0.f, -64.f)) && !previewLimitMap(m_Player.getPosition(), sf::Vector2f(0.f, -64.f)))
                {
                    m_Player.recoilUp();
                }
                else if(collide == UP_COLLIDE && !previewCollide(m_Player.getPosition(), sf::Vector2f(0.f, 64.f)) && !previewLimitMap(m_Player.getPosition(), sf::Vector2f(0.f, 64.f)))
                {
                    m_Player.recoilDown();
                }
                else if(collide == RIGHT_COLLIDE && !previewCollide(m_Player.getPosition(), sf::Vector2f(-64.f, 0.f)) && !previewLimitMap(m_Player.getPosition(), sf::Vector2f(-64.f, 0.f)))
                {
                    m_Player.recoilLeft();
                }
                else if(collide == LEFT_COLLIDE && !previewCollide(m_Player.getPosition(), sf::Vector2f(64.f, 0.f)) && !previewLimitMap(m_Player.getPosition(), sf::Vector2f(64.f, 0.f)))
                {
